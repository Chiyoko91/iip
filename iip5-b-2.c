...
void get_data(void)//ヘッダのデータを取得
{
  ...
  ins_byte=*(int *)&header[2]-*(int *)&header[10]-*(int *)&header[18]**(int *)&header[22]*(*(short *)&header[28]/8);//挿入バイト数

  //以下データ出力部
  //printf("＜ファイルタイプ＞\nheader[0]=%02x header[1]=%02x\n",header[0],header[1]);
  
  printf("\n＜ファイルサイズ＞\n");
  printf("\n%dバイト\n",filesize);
  
  //printf("\n＜予約領域＞\nheader[6]=%02x header[7]=%02x header[8]=%02x header[9]=%02x\n",header[6],header[7],header[8],header[9]);
  
  printf("\n＜オフセット＞\n");
  printf("\n%dバイト\n", *(int *)&header[10]);
  
  //printf("\n＜情報ヘッダサイズ＞\nheader[14]=%02x header[15]=%02x header[16]=%02x header[17]=%02x\n",header[14],header[15],header[16],header[17]);
  ...
  //Printf("\n＜色プレーン数＞\nheader[26]=%02x header[27]=%02x\n",header[26],header[27]);
  
  printf("\n＜１画素当たりのビット数＞\n");
  printf("\n%oバイト\n", *(short *)&header[28]);
  
  //printf("\n＜圧縮方式＞\nheader[30]=%02x header[31]=%02x header[32]=%02x header[33]=%02x\n",header[30],header[31],header[32],header[33]);
  
  //printf("\n＜画像データサイズ＞\nheader[34]=%02x header[35]=%02x header[36]=%02x header[37]=%02x\n",header[34],header[35],header[36],header[37]);
  
  //printf("\n＜水平解像度＞\nheader[38]=%02x header[39]=%02x header[40]=%02x header[41]=%02x\n",header[38],header[39],header[40],header[41]);
  
  //printf("\n＜垂直解像度＞\nheader[42]=%02x header[43]=%02x header[44]=%02x header[45]=%02x\n",header[42],header[43],header[44],header[45]);
  
  //printf("\n＜色数＞\nheader[46]=%02x header[47]=%02x header[48]=%02x header[49]=%02x\n",header[46],header[47],header[48],header[49]);
  
  //printf("\n＜重要な色数＞\nheader[50]=%02x header[51]=%02x header[52]=%02x header[53]=%02x\n",header[50],header[51],header[52],header[53]);
...
void rgb_to_ybr(void)
{
    ...
    for(int i=0;i<3;i++)
    {
        for(int j=0;j<width;j++)
        {
            ...
            {
                if(i==0) imgin[i][j][k]=limit(round_off(tmp_r2y[i][j][k]));
                else imgin[i][j][k]=limit(round_off(tmp_r2y[i][j][k])+128);
            }
        }
    }
...
void ybr_to_rgb(void)
{
    ...
    for(int i = 0; i < height; i++)
    {
        for(int j = 0; j < width; j++)
        {
            for(int k = 0; k < 3; k++)
            {
                tmp_y2r[0][i][j] += convert_rev[k][0] * imgout[k][i][j];
                tmp_y2r[1][i][j] += convert_rev[k][1] * imgout[k][i][j];
                tmp_y2r[2][i][j] += convert_rev[k][2] * imgout[k][i][j];
            }
        }
    }
    ...
    for(int i=0;i<height;i++)
    {
        for(int j=0;j<width;j++)
        {
            for(int k=0;k<3;k++)
            {
	            imgout[i][j][k]=(unsigned char)limit(round_off(tmp_y2r[i][j][k]));
            }
        }
    }
...
int round_off(double data)
{
    ...
    if (data>= 0.0) piyo=0.5;
    else piyo=-0.5;
    
    return (int)(data+piyo);
}
